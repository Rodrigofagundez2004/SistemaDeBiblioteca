...
class ParticipanteModel:
    ...
    def get_by_ci(self, ci):
        ...
        try:
            ...
            if cursor: cursor.close()
            if conn: conn.close()
    def create(self, ci, nombre, apellido, telefono, email):
        db = Database()
        conn = db.get_connection()
        cursor = None
        try:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO participante (ci, nombre, apellido, telefono, email) VALUES (%s, %s, %s, %s, %s)", (ci, nombre, apellido, telefono, email))
            conn.commit()
            return True, "Paricipante creado de forma exitosa."
    ...
    def update(self, ci, nombre, apellido, email):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE participante SET nombre = %s, apellido = %s, email = %s WHERE ci = %s", (nombre, apellido, email, ci))
            conn.commit()
